...
Dependencies: numpy, scikit-learn (optional for MLP); matplotlib only for plot_solution
...
class MASSSMASHConfig:
    ...
    # Vector mode: use antipodal_symmetry_scanner_vector for 3-component B field
...
def antipodal_symmetry_scanner_vector(
    B: np.ndarray,
    window_size: int = 40,
    ...
    """
    Vector-native antipodal symmetry scanner for multi-component magnetic field data.

    Extends the scalar antipodal scanner to a full D-component signal B ∈ ℝ^(T×D).
    This function is the candidate-generation step only; MDL scoring and
    run_mass_smash still operate on a 1D signal (whichever component the caller
    chooses to pass there).

    At each candidate position τ with half-window w = window_size // 2:

      B_L = B[τ-w:τ, :]   shape (w, D) — left window
      B_R = B[τ:τ+w, :]   shape (w, D) — right window

    AC component (oscillatory antipodal structure):
      Z-score each window independently per component (floor std at EPS),
      flatten to (w*D,), then compute cosine similarity of B_L_normed with
      -B_R_normed:
        corr_score = <B_L_flat, -B_R_flat> / (||B_L_flat|| * ||B_R_flat|| + ε)

    DC component (flat-plateau sign flip):
      μ_L = mean(B_L, axis=0), μ_R = mean(B_R, axis=0)
      polarity_score = 1 - ||μ_L + μ_R|| / (||μ_L|| + ||μ_R|| + ε)
      This is 1 when mean vectors are antipodal (μ_L ≈ -μ_R), 0 when aligned.

    Combined:
      A(τ) = 0.5 * corr_score + 0.5 * polarity_score   (clipped to [0, 1])

    Symmetric-pair refinement:
      For oscillatory sign-flip seams the score surface is often symmetric,
      producing peak pairs at approximately ±w from the true seam.  When two
      high-scoring local maxima are within 2*window_size of each other and nearly
      symmetric about a midpoint, that midpoint is injected as an additional
      candidate (with the mean score of the pair) before NMS.  This keeps the
      returned position within half_window samples of the true seam boundary.

    Note: run_mass_smash is unchanged and operates on 1D signals internally.
    Use detect_seam_candidates with a 2D B array (and config.vector_mode=True)
    to route candidate generation through this function.

    Args:
        B: Input array of shape (T, D), e.g. D=3 for [Bx, By, Bz]
        window_size: Full window size; each side uses window_size // 2 samples
        threshold: Minimum combined score to report as a candidate
        top_k: Maximum number of candidates to return
        min_separation: Minimum index distance between candidates (NMS)

    Returns:
        List of (index, score) sorted by score descending, same type as scalar scanner
    """
    ...
    try:
        # sliding_window_view for 2D array (T, D) with window shape (half, D)
        # gives output shape (T-half+1, 1, half, D); we squeeze dim 1
        wins = sliding_window_view(B, (half, D))  # (T-half+1, 1, half, D)
        wins = wins[:, 0, :, :]  # (T-half+1, half, D)
        ...
        Z_L = (W_L - mu_L[:, None, :]) / std_L[:, None, :]  # (N_valid, half, D)
        Z_R = (W_R - mu_R[:, None, :]) / std_R[:, None, :]

        Z_L_flat = Z_L.reshape(len(valid), -1)  # (N_valid, half*D)
        Z_R_flat = Z_R.reshape(len(valid), -1)

        dot = np.sum(Z_L_flat * (-Z_R_flat), axis=1)
        norm_L = np.linalg.norm(Z_L_flat, axis=1)
        norm_R = np.linalg.norm(Z_R_flat, axis=1)
        corr_score = dot / (norm_L * norm_R + EPS)

        combined = np.clip(0.5 * corr_score + 0.5 * polarity_score, 0.0, 1.0)
        scores[valid] = combined
        ...
        for i in range(half, T - half):
            ...
            # DC polarity score
            norm_sum = np.linalg.norm(mu_L) + np.linalg.norm(mu_R)
            pol = 1.0 - np.linalg.norm(mu_L + mu_R) / (norm_sum + EPS)

            # AC corr score with per-component std floor
            std_L = BL.std(axis=0)
            std_R = BR.std(axis=0)
            std_L = np.where(std_L < EPS, EPS, std_L)
            std_R = np.where(std_R < EPS, EPS, std_R)
            ...
            nL = np.linalg.norm(zl_flat)
            nR = np.linalg.norm(zr_flat)
            corr = float(np.dot(zl_flat, -zr_flat) / (nL * nR + EPS))

            scores[i] = float(np.clip(0.5 * corr + 0.5 * pol, 0.0, 1.0))
    ...
    # surface often produces symmetric peaks at ±w from the true boundary.
    # Inject the midpoint of near-symmetric pairs as additional candidates.
    sym_candidates = list(raw_candidates)
    search_radius = 2 * window_size
    if len(raw_candidates) >= 2:
        idxs = np.array([c[0] for c in raw_candidates])
        scs = np.array([c[1] for c in raw_candidates])
        injected = set()
        for a in range(len(raw_candidates)):
            for b in range(a + 1, len(raw_candidates)):
                sep = idxs[b] - idxs[a]
                if sep <= 0 or sep > search_radius:
                    continue
                mid = int(round((idxs[a] + idxs[b]) / 2.0))
                if mid in injected or not (half <= mid < T - half):
                    continue
                # Check rough symmetry: scores should be comparable
                if abs(scs[a] - scs[b]) / (max(scs[a], scs[b]) + EPS) < 0.3:
                    injected.add(mid)
                    # Give the midpoint a small score boost so it ranks above
                    # the individual pair members after sorting, ensuring NMS
                    # keeps the refined position rather than one of the flanking peaks.
                    pair_score = float(np.clip(max(scs[a], scs[b]) * 1.01, 0.0, 1.0))
                    sym_candidates.append((mid, pair_score))

    sym_candidates.sort(key=lambda x: x[1], reverse=True)
    return _nms(sym_candidates, min_separation, top_k)
...
def detect_seam_candidates(
    ...
    Accepts either:
    - 1D array (T,): uses antipodal_symmetry_scanner (scalar mode)
    - 2D array (T, D): uses antipodal_symmetry_scanner_vector when
      config.vector_mode=True or when the input is 2D

    Each detector's scores are normalized to [0,1] before merging to
    ensure fair comparison across detection methods.

    Note: run_mass_smash is unchanged and operates on 1D signals internally.
    The vector scanner is the candidate-generation step only; MDL scoring
    remains on whichever component the user passes to run_mass_smash.

    Returns:
        List of (index, score, detector_name) sorted by score
    """
    y = np.asarray(y)
    is_vector = y.ndim == 2 or config.vector_mode

    # Antipodal detector — scalar or vector
    if is_vector:
        # For roughness detector we need a 1D signal; use the vector norm
        y_1d = np.linalg.norm(y, axis=1) if y.ndim == 2 else y
        anti = antipodal_symmetry_scanner_vector(
            y,
