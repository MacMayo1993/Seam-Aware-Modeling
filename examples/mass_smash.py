...
def roughness_detector(
    ...
    for i in range(1, len(change_normalized) - 1):
        if (change_normalized[i] > threshold and 
            change_normalized[i] > change_normalized[i-1] and 
            change_normalized[i] > change_normalized[i+1]):
...
def get_segments(T: int, seams: Sequence[int], min_length: int) -> Optional[List[Tuple[int, int]]]:
    """
...
def plot_solution(
    ...
    ax.plot(t, solution.yhat, linewidth=2.5, color='green', 
            label=f'Fit (m={solution.n_seams} seams)')
    
    if true_seams:
        for i, ts in enumerate(true_seams):
            ax.axvline(ts, color='red', linestyle='--', linewidth=2, alpha=0.7,
                      label='True seam' if i == 0 else None)
    
    for i, seam in enumerate(solution.seams):
        ax.axvline(seam, color='blue', linestyle=':', linewidth=2.5,
                  label='Detected seam' if i == 0 else None)
    
    ax.legend(loc='best')
    ax.set_ylabel('Signal')
    ax.set_title(f'MASS/SMASH Solution | MDL={solution.total_mdl:.1f} bits | '
    ...
    if candidates:
        ...
        for idx, score, detector in candidates:
            ...
            ax.text(idx, 0.9, f'{score:.2f}', rotation=90, va='top', fontsize=8, color=c)
    ...
    ax.set_title('Seam Candidates (Green=Antipodal, Orange=Roughness, Blue=Selected)',
                fontweight='bold')
    ax.set_ylim([0, 1])
    ax.grid(True, alpha=0.3)
...
class BatchResults:
    ...
    def summary(self) -> str:
        lines = [
            ...
            "",
            "Seams detected:",
            f"  Distribution: {dict((k, self.n_seams_detected.count(k)) for k in sorted(set(self.n_seams_detected)))}",
            "",
            "Detection accuracy:",
        ]
        ...
        lines.extend([
            f"  Perfect (within 5%): {self.perfect_count}/{self.n_runs} ({100*self.perfect_count/self.n_runs:.1f}%)",
            "=" * 70,
        ])
...
def run_batch_evaluation(
    ...
    for seed in range(n_runs):
        # Generate signal
        seam_positions = [float(i+1)/(n_true_seams+1) for i in range(n_true_seams)]
...
if __name__ == "__main__":
    ...
    for i, sol in enumerate(all_solutions[:5]):
        print(f"  {i+1}. seams={list(sol.seams)}, transform={sol.transform}, "
