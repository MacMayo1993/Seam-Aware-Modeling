...
A detection is accepted only when the compression gain from modeling a
structural break exceeds this cost, providing a principled mechanism for
conservative event selection.

We validate MASS/SMASH against 120 known injected polarity-reversal discontinuities in
...
systematically offset by approximately $30\s$ from the true polarity center.
This offset arises from the finite chunked-inference procedure, MDL gain
maximization, and the forward-ordered antipodal correlation formula, rather
than from a physically distinct sub-feature of the tanh transition profile.
...
mean angles of $128^\circ$--$134^\circ$ at 30--120 s averaging windows,
...
is therefore a localization artifact, not evidence of different physical event
classes.
...
Simpler threshold methods on the scalar magnitude derivative $d|\mathbf{B}|/dt$
or on $|\mathbf{B}|$ directly are also widely used as rapid screeners.
...
A threshold on $\PVI$ or $d|\mathbf{B}|/dt$ will fire wherever the field magnitude
changes rapidly, regardless of whether the surrounding signal structure changes
character across the event or whether the full vector rotates coherently.
...
To our knowledge, MASS/SMASH is the first solar wind current-sheet detection
framework to combine an antipodal symmetry candidate score, a roughness
discontinuity detector, and explicit MDL-regularized seam acceptance.
We make no claim that this is the first application of MDL to time-series
changepoint detection; we do not claim superiority over PVI or the baseline
in all settings; we present MASS/SMASH as a structurally motivated complementary
selector with empirically characterized performance.
...
A note on the formula: equation~\ref{eq:antipodal} compares the
...
For the specific case of a sharp polarity reversal (nearly step-function in
character, narrow transition width relative to $w$), both windows are
approximately uniform --- one at $+B_0$, one at $-B_0$ after negation --- and
forward-ordered correlation equals mirror-reversed correlation to a good
approximation.
For wider transitions or asymmetric profiles, the two formulas differ.
We characterize the empirical localization behavior in
Section~\ref{subsec:offset} rather than making an analytic claim about which
...
which typically crosses at $\sim$1--2 per solar rotation; the synthetic rate is
consistent with smaller-scale flux-tube boundary crossings observed near 1 AU
...
$|d\mathbf{B}/dt|$: a polarity reversal can rotate $\mathbf{B}$ by $\approx 180^\circ$
while leaving $|\mathbf{B}|$ nearly constant, so $d|\mathbf{B}|/dt$ may not
fire strongly at a clean sector-boundary crossing.
We retain this detector as the simplest operational screener and acknowledge
that a vector-gradient or vector-PVI baseline would be a stronger comparator;
...
PVI performance is identical at both tolerance levels because PVI peaks the
field gradient at the tanh transition center, which the injected catalog also
records at the tanh midpoint; the two are within a few samples of each other.
...
in the PVI catalog (i.e., crossings PVI missed).
...
The net result is a localization precision of approximately $\pm 30\s$ --- one
order of magnitude better than the chunk step size, but not sub-transition-width
precision.

For applications requiring precise polarity-center localization, a
post-detection profile-fitting step (e.g., fitting a tanh model to the Bz
segment around each detection) would recover sub-10-s accuracy.
...
The $\approx \pm 30\s$ localization uncertainty of MASS/SMASH relative to the
injected polarity center arises from algorithmic discretization (the MDL chunk
step size is 300 s but the seam position within each chunk is resolved to
individual samples) and from the forward-ordered antipodal correlation formula,
which does not guarantee detection at the exact signal midpoint for wide
transitions (Section~\ref{subsec:antipodal}).
The offset is not associated with a physically meaningful sub-feature of the
transition profile; it is an implementation artifact.

Practically, the $\pm 30\s$ precision is sufficient for most solar wind survey
applications: at $\sim$4 events/day the typical inter-event spacing is
$\sim$6 hours (21{,}600 s), so the $\pm 30\s$ uncertainty is
$\approx 0.14\%$ of the typical inter-event interval.
For applications where transition morphology matters --- measuring current sheet
thickness, reconnection jet width, or onset-to-peak delay --- a post-detection
tanh profile fit constrained by the MASS/SMASH detection position and MDL gain
score would provide sub-10-s localization.

\subsection{Complementarity with PVI}
\label{subsec:comp}

PVI and MASS/SMASH together would detect 114 of 120 injected crossings --- 95\%
of the ground-truth population --- with a combined false discovery fraction of
approximately 5\% (6 false positives from 1 + 5, out of $\approx$120 combined
detections).
...
In this synthetic benchmark the $d|\mathbf{B}|/dt$ baseline already achieves
perfect recall, so MASS/SMASH cannot add coverage.
However, in real data --- where the baseline false discovery fraction may be
higher and no injected ground truth exists --- MASS/SMASH could serve as a
\emph{follow-on quality scorer}: applied to an initial baseline catalog, it
provides a continuous MDL gain score that ranks events by structural evidence.
...
reversal amplitude in Bz (field direction $\hat{\mathbf{b}}_0 = (0.20, 0.20,
0.96)$), which makes the benchmark favorable for Bz-based detection.
A follow-up benchmark should randomize the field orientation so that the
reversal amplitude is distributed across components, testing performance on
oblique crossings where no single component dominates.
...
\item \textbf{MASS/SMASH detections have a localization precision of $\approx \pm 30\s$
relative to the injected polarity center.}
This empirical uncertainty arises from the finite MDL chunk step size and the
forward-ordered (non-mirror-reversed) antipodal correlation formula.
It is predictable and correctable via post-detection profile fitting, and the
MDL gain score could prioritize which events warrant that refinement.
